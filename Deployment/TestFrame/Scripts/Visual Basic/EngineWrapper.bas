Attribute VB_Name = "EngineWrapper"
'################################################################################
...
'*                  Notepad showing error.log, the file that has the details about why the
...
( _
    ...
    'StartApplication "C:\WINDOWS\NOTEPAD.EXE " & CurDir & "\error.log"
...
( _
	...
	strIniFile As String, _
	oNavigation As Object _
) As Integer

    Dim nResult As Integer      ' result
    ...
    If nResult = 1 Then
        ...
        Do While tfe_ProcessNextLine <> 0

            ' Execute the script containing action word function.
            Call CallByName(oNavigation, tfe_GetActionWordFunction, VbMethod)

        Loop

        ' Stop the engine and generate the report.
        Call tfe_StopEngine
        Call tfe_GenerateReport
    Else
